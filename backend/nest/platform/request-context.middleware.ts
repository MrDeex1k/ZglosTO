...
import { addCounter, recordHistogram, withHttpServerSpan } from '@zglosto/observability';
...
export class RequestContextMiddleware implements NestMiddleware {
  ...
  use(request: Request, response: Response, next: NextFunction): void {
    ...
    const enterRequestPipeline = (): void => {
      this.correlationContext.run(correlationId, requestTraceparent(request), () => {
        response.once('finish', () => {
          ...
          recordHistogram(
            'zglosto_http_server_duration_seconds',
            durationSeconds,
            metricAttributes,
          );
          this.logger.log(
    ...
    void withHttpServerSpan(
      {
        method: request.method,
        headers: new Headers({ traceparent: requestTraceparent(request) ?? '' }),
      },
      () =>
        new Promise<{ status: number }>((done) => {
          response.once('finish', () => done({ status: response.statusCode }));
          response.once('close', () =>
            done({ status: response.writableFinished ? response.statusCode : 499 }),
          );
          enterRequestPipeline();
        }),
      // Forward asynchronous span/pipeline failures to Express error middleware.
