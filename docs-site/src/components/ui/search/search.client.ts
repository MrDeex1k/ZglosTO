...
export function initSearch(config: SearchConfig): SearchInstance {
  ...
  async function ensureInitialized(): Promise<boolean> {
    ...
    try {
      ...
      emptyState.textContent =
        'Wyszukiwanie jest dostępne w podglądzie po wykonaniu bun run build:docs.';
      return false;
    }
  }
  ...
  async function runSearch(query: string): Promise<void> {
    ...
    emptyState.textContent = 'Wyszukiwanie…';
    clearResults();
    ...
    try {
      ...
      if (results.length === 0) {
        emptyState.style.display = '';
        emptyState.textContent = 'Nie znaleziono wyników.';
        return;
      }
      ...
      clearResults();
      emptyState.style.display = '';
      emptyState.textContent = 'Wyszukiwanie jest chwilowo niedostępne.';
    }
  }
  ...
  function handleInput(): void {
    ...
    debounceTimer = setTimeout(() => {
      ...
      if (!query) {
        ...
        clearResults();
        emptyState.style.display = '';
        emptyState.textContent = 'Wpisz szukaną frazę…';
        return;
      }
